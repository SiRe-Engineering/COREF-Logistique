...
Write-Host "=== Lot AB.1 - Fiabilisation des tests backend ===" -ForegroundColor Cyan
...
docker compose exec backend sh -lc "cd /app && python -m pytest --collect-only -q" 2>&1 | Tee-Object -FilePath ".\AB1-PYTEST-COLLECT.txt"

Write-Host "`n--- Suite Pytest ---" -ForegroundColor Cyan
docker compose exec backend sh -lc "cd /app && python -m pytest -q" 2>&1 | Tee-Object -FilePath ".\AB1-PYTEST-RESULTATS.txt"
